...
class Scene3Monitor:
    ...
    截图：入库时不截图，直接标记待补截，由补截循环分批补齐（首次大量入库暂缓）。
    """
    ...
    def __init__(self, db: Database, client: BiliClient, config: Config, screenshotter=None):
        ...
        self.client = client
        self.config = config
        self.screenshotter = screenshotter  # 保留字段（与场景2签名一致，供后续扩展）
    ...
    async def discover(self, clip_up: ClipUpConfig):
        ...
        - 新入库项直接标记待补截（首次大量入库暂缓截图，由补截循环分批补齐）
        ...
        for dyn in dynamics:
            ...
            if inserted:
                new_count += 1
                # 暂缓截图：先标记待补截，由补截循环分批补齐（避免首次大量入库阻塞发现流程）
