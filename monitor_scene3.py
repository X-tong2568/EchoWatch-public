# monitor_scene3.py
"""EchoWatch 场景三：监测切片UP主投稿评论区中目标UP主（星瞳）的回复"""
...
class Scene3Monitor:
    """
    场景三：监测切片UP主投稿视频的评论区，匹配目标UP主（星瞳）的评论/回复。

    与场景一/二的区别：
    - 匹配对象是配置的 target_uid（星瞳），不是作品归属者（切片员）
    - 不检查 up_action.like（那是切片员的点赞，不是星瞳的互动）
    - 不监测置顶/priority（切片员的置顶与星瞳回复无关）

    两阶段轮询（同场景一）：
    - 阶段A：拉取切片员空间动态列表 → 新视频入库（source="scene3"）
    - 阶段B：轮询监测队列中各视频的评论区，匹配星瞳的评论/回复
    ...
    def __init__(self, db: Database, client: BiliClient, config: Config, screenshotter=None):
        ...
        self.target_uid = config.scene3.target_uid  # 目标UP主UID（星瞳）
    ...
    async def sweep_sub_comment_baselines(self):
        ...
        rows = await self.db.get_all_sub_baselines(source="scene3", up_uids=list(clip_uids))
        if not rows:
            return
        ...
        logger.info(
            f"场景三子评论基线扫查: 基线{len(rows)}条 翻页{checked}条 过期同步{synced}条"
        )
