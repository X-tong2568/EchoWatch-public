## v2.3 (2026-09-05) — 评论发送者昵称徽章 + av 降级项富化

### 修复

- **「UP觉得很赞」邮件徽章误用 UP 名**：v1.8.1 起评论内容前打【UP名】标签，但被赞评论的作者是粉丝不是 UP。
  interactions 表新增 comment_author 列（旧库自动迁移），全部场景入库真实发送者昵称（uname），
  邮件统一显示【发送者昵称】；老数据无该字段时降级 UP 名兜底。
- **av 降级项邮件截图截到视频播放页**：空间动态 API 风控期间经视频搜索降级入库的 av{aid} 项只有 aid，
  截图器只能截播放页。新增富化机制：feed 恢复成功的轮次按 aid 匹配已入库 av 项，升级为真实动态 ID
  （monitored_items + interactions + sub_comment_baseline 三表联动），推送前缓存命中也零成本兜底；
  此后截图/链接走正常动态页。仅 scene1 生效（scene3 切片视频的 av 前缀是常态，不富化）。

---
## v2.2 (2026-09-04) — priority 通知徽章

### 新增

- **priority 徽章**（`email_notifier.py` + `config.py`）：priority（置顶动态）互动的邮件卡片标题行新增徽章，一眼识别"这是置顶动态的互动"——
  - 单封即时通知（priority 主评论）：类型徽章后附加徽章（`主评论` + `优先通知`）
  - priority 子评论批量邮件 / 日报：卡片标题行 `#N` 后附加徽章
    （`#1 优先通知 2026-09-03 15:09:19 自身作品 主评论`）
  - 徽章样式使用邮件随机主题渐变（与场景标签徽章一致）
- **配置项**（`config.py` + 示例配置）：`email.priority_badge_text`（默认"优先通知"）

### 效果

- 上报的互动属于 priority 时邮件卡片一眼可辨；非 priority 内容不受影响（不传集合即不渲染）

---

## v2.1.1 (2026-09-02) — 场景四分级轮询 / 流量优化

### 变更

- **场景四发现循环分级化**（`scheduler.py`）：按 UP 活跃度分两档拉取空间动态——
  - 近期有动态（该 UP 存在 Level 1 帖，24h 内发过的）→ 30 分钟一次
  - 长期无动态（Level1=0）→ 18 小时一次（新动态最迟延迟 18h 入库，
    入库按 `pub_ts` 定级，不会把老动态当新互动误报）
  - 实现：60s 心跳逐 UP 判断到期，重启后首轮全量拉一次（与旧实现单轮等价）；
    活跃判据查 DB 一次拿全量 L1 帖按 `up_uid` 分组，不重复查询
- **场景四 L1 评论轮询降频**（`scheduler.py`）：每 5 分钟 → 30 分钟一次
  （`level1_poll_seconds` 不变，场景一/二/三不受影响）
- **配置项**（`config.py` + 示例配置）：`scene4_discover_active_seconds`(1800)、
  `scene4_discover_idle_seconds`(64800)、`scene4_poll_seconds`(1800)，缺省即生效，
  `config.yaml` 无需变更
- 效果预估：场景四空间列表请求从约 8400 次/天 降至约 350 次/天（-95%）
...
- **删除"入库即截图"逻辑**（`monitor_scene1.py` / `monitor_scene2.py` /
  `monitor_scene3.py` / `monitor_scene4.py` 发现循环）：
  此前发现层每篇新帖入库即截图（每轮最多 5 张，超限标记待补截，补截循环每 10 分钟
  分批补齐直到成功）。但绝大多数被发现的动态根本不会出现目标UP的评论——
  这些截图 100% 白截，还要承担失败重试、页面风控重试的额外流量，是 2026-08-28
  机场流量耗尽事故（12GB/天）的流量主体。v2.0 起发现层只入库数据，不再触发浏览器。
- **截图改到推送时按需补截**（`scheduler.py` `_retry_screenshot_before_send`）：
  只有真产生了目标互动的帖子才截图（"用哪付哪"），本批有多少互动就截多少
  （不设上限——截图是最终内容兜底，B站 纯文本提取常为空白，见已知限制；
  互动数本身有界，每张约 2~5s，多等几秒无妨），单张失败/超时则该条空着发送
  ——邮件自动降级文本渲染，不阻塞不积压；已截过的直接复用（文件存在即跳过）。
- **日报二次兜底**：发送前对"推送时没截到"的互动再补一次原帖截图（终版汇总
  尽力补全；已有截图直接复用，不重复截取同一页），失败仍文本降级。
- **补截循环（`retry_screenshots`）自然空转**：不再产生新的 `screenshot_pending`
  标记，存量清理完后该循环永久空闲；`max_per_batch` / `retry_interval` 配置项
  仅兼容保留。

### 效果

- 截图流量从约 1~2GB/天 降至约 0.1~0.3GB/天（每天只截 20~40 张互动相关帖）；
  由于不存在待补截积压池，"风控降级拖入历史投稿"等极端场景不再可能烧掉流量
- 邮件时延：每次发送最多增加几秒补截时间（每张约 2~5s）

---

## v1.9.4 (2026-08-28) — 老帖截图守卫（风控降级历史投稿不再截图/待补截）

### 修复

- **风控降级拉入的历史投稿不再截图、不再标记待补截**（`monitor_scene1.py` /
  `monitor_scene3.py` / `monitor_scene4.py` 发现循环）：
  空间动态 API 风控时降级到视频搜索接口，会把 UP 主的历史投稿（含数年前）整批拉回；
  此前这些老帖（发布超 120h，入库即归档 Level 0）仍会被标记 `screenshot_pending=1`，
  涌入补截循环后形成庞大积压（实测 829 张，其中 825 张超 7 天、最早 2020 年入库）。
  补截循环每 10 分钟补 5 张，每次截图都是一次经代理的完整页面加载（3~8MB），
  叠加失败重试后代理机场流量被推高至 12GB/天。修复：
  - 发布超过 120h（`thresholds.level2_hours` 口径，与 v1.9.1 入库分级一致）的老帖
    入库后不截图、不标记待补截——它们不会进轮询队列，原帖截图无邮件用途
  - 时间戳解析兼容 B站 API 返回形态（字符串/浮点均转纯整数），解析异常按 0
    处理（= 按新帖处理，安全兜底，不打断发现流程）
...
- **风控熔断器**（`bili_client.py` + `config.py` + `main.py`）：
  出口 IP 被 B站 -352/-412 风控时，日志曾出现"边被风控边疯狂降级"的连锁反应
  （37分钟 222 次 space feed banned + 63 次视频搜索降级，空间动态+降级搜索轮询
  同时刷满 CPU/内存）。现新增熔断机制：
  - 连续 3 次风控命中（`RATELIMIT_BREAKER_HITS`）触发熔断，冷却时长由配置
    `breaker.ratelimit_cooldown_seconds` 控制（默认 1800s = 30 分钟）
  - 熔断期内空间动态接口**零请求**，直接走缓存兜底（`_dyn_cache`）/返回空列表，
    不再触发视频搜索降级 → 风控状态期间不再自我放大请求量
  - 冷却到期自动解除（日志打印"风控熔断解除，恢复空间动态轮询"），无需重启
- 新配置段 `breaker:`（config.example.yaml 模板同步）：
...
- **风控降级路径拖入历史投稿后，历史评论被当作新互动推送**
  （`database.py` + 四个场景 `_poll_item`）：
  空间动态 API 风控时降级到视频搜索接口，会把 UP 主的历史投稿（含数年以前的）整批带回；
  旧帖入库时 `upsert_item` 将 `monitor_level` 硬编码为 1（Level 1），且轮询时降级
  L1→L2 后仍继续扫描评论区，导致历史帖当轮即被完整扫描，帖下积累的历史评论/
  历史"觉得很赞"被当作新互动生成了通知。修复：
  - `upsert_item` 初始级别按发布时间年龄计算：发布超 120h 入库即归档（不进轮询队列）、
    24~120h 为 Level 2、24h 内为 Level 1；priority 项不受影响（恒为 Level 1）
  - 四个场景 `_poll_item` 级别变更后本轮结束（下一轮按新级别处理），
    不再"降级后顺带扫描评论区"
- **av 前缀降级项截图生成超长图**（`screenshotter.py`）：视频搜索降级路径的 item_id
  带 av 前缀，截图访问的是视频播放页（该页无动态卡片元素），原有降级分支执行全页
  截图，侧边推荐视频一起被截入，图片过度拉长；现改为 av 降级项只截视口
  （播放器+标题区，固定尺寸），动态页仍保持原行为

---

## v1.9.0 (2026-08-26) — 场景四：其他UP动态评论区监测

### 新增

- **场景四：其他UP主动态/投稿/专栏评论区中目标UP主的评论/回复监测**
  （`monitor_scene4.py` 新文件，结构与场景三一致）
  - 配置 `scene4: {target_uid, other_up_list}`，填写形式与场景三相同（name 留空启动时自动获取）
  - 复用空间动态接口，动态/视频投稿/专栏全部类型覆盖；匹配 `mid == target_uid`，不检查 `up_action.like`
  - 分级 L1（0-24h）→ L2（24-120h）→ 归档，无 priority；子评论基线节流 + 尾部窗口翻页 + 兜底扫查与场景三一致
  - 通知：批量合并一封（每 180s，`send_scene4_batch`），邮件带原帖上下文卡片（截图优先），
    子评论附被回复主评论与富文本；标题「在其他UP动态有新互动」
  - 跨场景去重：comment_oid 维度先到先得（场景一不受限，联合投稿场景由场景一先手）
- **config / main / scheduler / email_notifier 全面接线**：场景开关 `scene4_enabled`、
  其他UP昵称自动获取、截图器/补截循环启动条件扩展至场景四

### 变更

- **场景三 discover 截图策略统一为混合策略**（与场景一/四一致）：入库时小批量
  （≤单批上限）立即截，超限暂缓标记待补截，由补截循环分批补齐（此前为全部暂缓等待补截循环）
- email_notifier 场景标签/原帖卡片/正文查询支持场景四（场景标签新增「其他UP动态」）
